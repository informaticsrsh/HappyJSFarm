...
document.addEventListener('DOMContentLoaded', () => {
    ...
    DOM.storeItems.addEventListener('click', (e) => {
        ...
        if (amountType === '1') {
            amount = 1;
        } else if (amountType === '10') {
            amount = 10;
        } else if (amountType === '100') {
            amount = 100;
    ...
    DOM.refModal.addEventListener('click', (e) => {
        if (e.target.classList.contains('tab-btn')) {
            const tab = e.target.dataset.tab;
            DOM.refModal.querySelector('.store-tabs .active').classList.remove('active');
            e.target.classList.add('active');

            DOM.refModal.querySelectorAll('.tab-content').forEach(c => c.classList.remove('active'));
            document.getElementById(`${tab}-content`).classList.add('active');
        }
    });
